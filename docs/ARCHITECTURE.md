...
- for Codex SSH, a temporary `$WORKSPACE_PATH/.agent-sandbox-codex-ssh` mountpoint so Git SSH config remains reachable from Codex's native sandbox; the launcher adds it to local `.git/info/exclude`
